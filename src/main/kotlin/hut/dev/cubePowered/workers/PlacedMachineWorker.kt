...
internal object PlacedMachineWorker {
        ...
        fun handleNewPlacedMachine(e: CustomBlockPlaceEvent, plugin : Plugin)
        {
            ...
            if(foundDuplicate == false) {
                ...
                val guiInstanceToAdd = Gui.storage().title(Component.text(placedMachineInstanceToAdd.machineTitle)).rows(placedMachineInstanceToAdd.guiRows!!).create()
                ...
                Lists.placedMachinesGuiInstances[placedMachineInstanceToAdd.machineKey] = guiInstanceToAdd//Assign value to key on GUIs mapping
    ...
    fun handleBrokenMachine(e: CustomBlockBreakEvent, plugin: Plugin)
    {
        ...
        val blockZ = e.block.z
        val blockWorld = e.block.world.name
        var machineToRemoveIndex = -1
        for(machineToRemove in Lists.placedMachinesInstances)
        {
            if(machineToRemove.machineX == blockX && machineToRemove.machineY == blockY && machineToRemove.machineZ == blockZ && machineToRemove.machineWorld == blockWorld)
            {
                machineToRemoveIndex = Lists.placedMachinesInstances.indexOf(machineToRemove)
                break
            }
        }
        if(machineToRemoveIndex != -1)
        {
            val placedMachineToRemove = Lists.placedMachinesInstances.removeAt(machineToRemoveIndex)
            plugin.logger.info("Successfully removed machine " + placedMachineToRemove.machineId + " from memory registry " + " Loc:[" + placedMachineToRemove.machineX + " " + placedMachineToRemove.machineY + " " + placedMachineToRemove.machineZ + " " + placedMachineToRemove.machineWorld + "]")
        }
    }

    fun handlePlacedMachineInteract(e: CustomBlockInteractEvent, plugin: Plugin)
    {
